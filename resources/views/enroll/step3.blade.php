...
@section('content')
    @php
        // 1) Détection formule
        $planName = mb_strtolower($enrollment->plan->name ?? '');
        $isAmateur = str_contains($planName, 'amateur');

        // 2) Dernier paiement + meta
        $payment = $enrollment->payments->last();
        $meta = $payment?->meta ?? [];

        $gross = $meta['gross_cents'] ?? ($enrollment->plan->price_cents ?? 0);
        $aid   = $meta['aid_cents']   ?? 0;
        $net   = $meta['net_cents']   ?? ($payment?->amount_cents ?? max(0, $gross - $aid));

        // 3) Liste des documents selon la formule
        //    (clé => label, value => chemin asset)
        if ($isAmateur) {
            $docs = [
                'Certificat médical (amateur)' => 'assets/docs/certificat-medical.pdf',
                'Certificat ophtalmologique (amateur)' => 'assets/docs/certificat-ophtalmo.pdf',
            ];
        } else {
            $docs = [
                'Certificat médical (Aéro / BEAL / Handi / Vétéran)' => 'assets/docs/Certificat-Medical-AERO-BEAL-HANDI-VETERAN-2024-2025.pdf',
            ];
        }
    @endphp
            ...
            <section class="attest-body">
                ...
                <div class="grid-2">
                    ...
                    <div class="box">
                        ...
                        <div class="v">{{ number_format(($enrollment->plan->price_cents ?? 0)/100, 2, ',', ' ') }} €</div>
                    </div>
                </div>
                    ...
                    <div class="dl-list">
                        @foreach($docs as $label => $path)
                            <a class="dl-link {{ $isPaid ? '' : 'is-locked' }}"
                               href="{{ $isPaid ? asset($path) : '#' }}"
                               @if(!$isPaid) aria-disabled="true" tabindex="-1" @endif
                               @if($isPaid) download @endif>
                                {{ $label }}
                            </a>
                        @endforeach
                    </div>
